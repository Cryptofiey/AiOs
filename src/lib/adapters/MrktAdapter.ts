...
export class MrktAdapter implements MarketAdapter {
  public readonly name = "MRKT";
  ...
  public async fetchLatestListings(): Promise<any[]> {
    ...
    this.isOnline = true;
    try {
        const messages = await this.bridge.getLatestMessages("@main_mrkt_bot", 40);
        if (messages && messages.length > 0) {
            for (const msg of messages) {
                const parsed = this.parseMrktMessage(msg);
  ...
  public normalizeData(rawData: any): NormalizedOrder | null {
    ...
    return {
      ...
      category: rawData.category || (rawData.type === "BID" ? "ORDER" : "MARKET"),
