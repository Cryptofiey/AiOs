...
export class TonnelAdapter implements MarketAdapter {
  ...
  public async fetchLatestListings(): Promise<any[]> {
    ...
    if (!status || status.status !== "connected") {
        this.isOnline = false;
        console.warn(`[${this.name}Adapter] MTProtoBridge not connected. Cannot fetch from @Tonnel_Network_bot`);
        return [];
    }
    ...
    try {
        ...
        this.isOnline = false;
        console.error(`[${this.name}Adapter] Error fetching from @Tonnel_Network_bot:`, e.message);
    }
    return allResults;
  }
