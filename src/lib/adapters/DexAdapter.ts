...
export class DexAdapter implements MarketAdapter {
  ...
  public async fetchOrderBook(itemName: string): Promise<{ bids: NormalizedOrder[]; asks: NormalizedOrder[] }> {
    ...
    const match = orders.find(o => o.metadata.itemName.toLowerCase() === itemName.toLowerCase());
    
    // For AMMs like DeDust, the order book is continuous based on the bonding curve.
    // We could calculate depth, but for simplicity, we return empty arrays if no pool is found,
    // or just the current price as a single bid/ask.
    if (!match) return { bids: [], asks: [] };
