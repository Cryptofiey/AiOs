...
import { 
  ...
  Clock, RefreshCw, BarChart2, Zap,
  Layers, Percent, Shield
} from "lucide-react";
...
export const MarketOverview: React.FC = () => {
  ...
  const [flashTick] = useState<boolean>(false);
  const [lastTickPrice, setLastTickPrice] = useState<number | null>(null);
  const [lastTickDiff] = useState<number>(0);
  ...
  // Calculate display data based strictly on real spot prices.
  // Drift/Simulated bid-ask lines have been removed to comply with 'No Simulation' mandates.
  const chartData = currentGift.data.map((d) => {
    return {
      ...d,
      bid: d.price, // Fallback to spot price
      ask: d.price, // Fallback to spot price
    };
  });
