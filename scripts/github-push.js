...
async function pushToGitHub() {
  ...
  try {
    ...
    try {
      ...
      // Self-healing: Remove index lock if it exists
      const lockPath = path.join(dir, ".git", "index.lock");
      if (fs.existsSync(lockPath)) {
        try {
          fs.unlinkSync(lockPath);
          console.log("[GitHub Auto-Push] Removed stale Git index lock.");
        } catch (e) {}
      }
      ...
      try {
        console.log("[GitHub Auto-Push] Adding files to Git...");
        execSync("git add .", { stdio: "inherit" });
      } catch (e) {
        console.warn("[GitHub Auto-Push] Warning: git add . failed, trying git add -A...");
        execSync("git add -A", { stdio: "inherit" });
      }
      
      try {
        const msg = `Auto-commit from AI Studio build - ${new Date().toISOString()}`;
        execSync(`git commit -m "${msg}"`, { stdio: "pipe", encoding: 'utf8' });
        ...
        if (output.includes("nothing to commit") || output.includes("working tree clean")) {
          console.log("[GitHub Auto-Push] No changes to commit (working tree clean).");
        } else {
          console.log("[GitHub Auto-Push] Commit skipped (working tree clean or empty stage).");
        }
      }
    ...
    console.log("[GitHub Auto-Push] Native git failed or skipped. Attempting isomorphic-git push fallback...");
